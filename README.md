...
### Optional Vision-native export

After a sort, the fork can compute electrical images from that sort's own
spike arrays and write a basename-consistent Vision bundle. This is separate
from sorting and requires the lab `kilosort1` environment's optional
`visionwriter` package:

```bash
~/anaconda3/envs/kilosort1/bin/python tools/export_vision.py \
  /path/to/kilosort4 /path/to/raw/data007 \
  -o /path/to/vision/kilosort4 --mode exact
```

The bundle contains `kilosort4.neurons`, `kilosort4.globals`,
`kilosort4.ei`, and `vision_export.json`. Kilosort cluster IDs are converted
from zero-based to one-based Vision cell IDs. Raw Litke channel-0 TTL
transition indices are written to Vision's reserved TTL table in the
`.neurons` file; channel 0 is not treated as a neural cell. `.sta` and
`.params` remain stimulus-analysis outputs and are generated by the Java
Vision STA route when required.
